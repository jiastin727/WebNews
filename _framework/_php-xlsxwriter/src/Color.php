...
class Color {
    ...
    private static function stringToHex(string $str) : string {
        $code = self::getHexCode(crc32($str));
        $code = substr($code, 0, 6);
        return $code;
    }

    private static function rgbToHex(string | int $red, string | int $green, string | int $blue) : string {
        return self::getHexCode($red) . self::getHexCode($green) . self::getHexCode($blue);
    }

    private static function getHexCode(string | int $code) : string {
        return str_pad(dechex($code), 2, 0, STR_PAD_LEFT);
    }

    /*
	 * @param $from string [rgb, hex, string]
	 * @param $value string | int [rgb: array, hex: string, string: string]
     * @return $colorCode string
	 * */
    public static function getColorCode(string $from, string | array $value) : string {
        $colorCode = "FF";
        if($from === "rgb") {
            $red = $value["red"] ?? 0;
            $green = $value["green"] ?? 0;
            ...
            $colorCode .= self::COLORS[$value] ?? self::stringToHex($value);
        }
