      ...
      if (isEmpty(obj)) {
           ...
           )
        {
          ...
          for (a = 0; a < jmlDataa; a++) {
            var v_idc = obj['read_content'][a]['c_id'];
            var myarr  = new Array();
            myarr  = v_idc.split(",");
            var myarr2  = new Array();
            myarr2  = idvalnotok.split(",").map(Number);
            var myarr3  = new Array();
            myarr3  = idvalx.split(",").map(Number);
            // notok = myarr2.length;
            // for(s = 0; s < notok; s++){
              
              // console.log(myarr2);
              if((idvalnotok.indexOf(myarr) > -1)){
                if(section === "ELECTRONIC"){
                  $(TC).append(
              ...
              else if((idvalnotok.indexOf(myarr) > -1)){
                if(section === "ELECTRONIC"){
                  $(TC).append(
                    "<div class='active title'>"+
                      "<i class='dropdown icon'></i>"+obj["read_content"][a]['c_name']+" &nbsp;&nbsp;&nbsp;<span class='EditGroupFieldLv1 BtnEditLv1"+section+"' sect='"+section+"' idx='"+obj['read_content'][a]['c_id']+"' nm='"+obj['read_content'][a]['c_name']+"'><i class='edit small whiteicon icon '></i></span>"+
              ...
              }else if((idvalx.indexOf(myarr) > -1)){
...
function FungsiSignPIC(){
  ...
  $('.BtnSignPIC').on('click', function() {
    ...
    $.ajax({
      type    : "POST",
      url     : `../RND_BackEnd/backend_bp/bp/load-checkerresult`,
      headers : {
        user  : vusrnm,
        token : vtoken
      },
      ...
      success: function (response){
        ...
        if(isEmpty(obj)) {
          ...
          jmlData = obj.length;
          for (a = 0; a < jmlData; a++) {
            var filelink = "<a target='_blank' href='../RND_Upload/BP/RESULT/"+obj[a]["NamaFile"]+"'>";
...
$('.CancelData').click(function(){
  ...
  $('#BtnHistoryPSC').click(function(){
    $.ajax({
      type    : "POST",
      url     : `${restlocpsc}load_datarevisi`,
      headers : {
        user  : vusrnm,
        token : vtoken
      },
