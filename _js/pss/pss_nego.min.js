...
function TableListFunc(FSearch, FYear, FNama, FJenis, FProgress, FStatus) {
      ...
      { data: null,
        render: function (data, type, row) {
          if (data[0] != "") {
            if (
              data[15] == Udecrypt ||
              (data[15] == Udecrypt && data[17] !== "NEGOSIATOR")
            ) {
              if (data[18] !== "FINISH" && data[18] !== "CLOSE") {
                return `<span class="editpss clicklink" eid="${data[0]}" edesc="${data[6]}" data-tooltip="Edit PSS" data-inverted="" data-position="right center"><i class="pencil alternate link editpssheader circular teal icon"></i></span>`;
              } else {
                return "";
              }
            } else {
              return `<span class="editpss clicklink" eid="${data[0]}" edesc="${data[6]}" data-tooltip="View Detail PSS" data-inverted="" data-position="right center"><i class="tasks link circular grey icon "></i></span>`;
            }
          } else {
            return "";
          }
      ...
      { data: null,
        render: function (data, type, row) {
          if(data[15] == Udecrypt) {
            ...
            if ((data[21] == null) || (!data[21])) {
              return `<i class="sticky note outline grey icon"></i>`;
            } else {
              return  data[21].length > 20 ? ''+data[21].substr( 0, 20 )+'...' : data[21];
            }
          }  
        }
      },
  ...
  $("#PSSList").on("click", ".editpss", function () {
    ...
    window.location.href = "./?link=pssitemnego&i=" + eidENC;
  });
