      ...
      if (isEmpty(obj)) {
        ...
        for (a = 0; a < jmlData; a++) {
          ...
          if (
            ...
            if (obj[a]["no_itempss"] == obj[a]["refer_noitem"]) {
              var delitem = "";
              // var addplus = "<i class='plus square outline circular actadditem iconcoloradd icon' id='"+obj[a]["id_item"]+"'></i>";
              // var addplus ="<span class='actadditem' id='" +obj[a]["id_item"] +"' data-tooltip='Add new sub-item Negosiator' data-inverted='' data-position='top left'><i class='plus square outline link circular red icon'></i></span>";
              var addplus ="";
              var subitem = "";
...
$(document).ready(function () {
  ...
  $(SignMSC).on("click", function (e) {
    var idpss = PSSID;
    var nego = $("#HeaderNegosiator").val();
    var user = Username;

    $.ajax({
      type: "POST",
      url: `${restlocpssneg}sign_pssnegosiatorshd`,
      headers: {
        user: vusrnm,
        token: vtoken,
      },
      data: { idpss: idpss, user: user, nego: nego },
      cache: false,
      success: function (response) {
        var obj = JSON.parse(JSON.parse(response));
        if (isEmpty(obj)) {
