  ...
  $.ajax({
    ...
    success: function (response){
      ...
      for (a = 0; a < jmlData; a++) {
        ...
        $('#pldate').html('Date Sign: '+moment(date_pl).format('ddd, DD MMM YYYY'));
        $('#headofgroupdate').html('Date Sign: '+moment(date_hog).format('ddd, DD MMM YYYY'));

        // DQA Sign Check
        if(sign_hod =="SIGN" ){
          $('#headofdqadate').html('Date Sign: '+moment(date_hod).format('ddd, DD MMM YYYY'));
      ...
      if(sign_hod =="SIGN"){
        if(sign_release == "NOT RELEASE"){
          ...
          $('#releasedate').html('Date Sign: '+moment(date_release).format('ddd, DD MMM YYYY'));
...
function LoadDataContent(section){
  ...
  $.ajax({
    ...
    success: function (response){
      ...
      // if(section =="CYCLE"){
      //   var MsgCy = obj['read_sectionresult']['message'];
      // } else if(section =="ELECTRONIC"){
      //   var MsgEl = obj['read_sectionresult']['message'];
      // } else if(section =="MD"){
      //   var MsgMD = obj['read_sectionresult']['message'];
      // } else if(section =="ID"){
      //   var MsgID = obj['read_sectionresult']['message'];
      // } 

      // if(groupprd =="WHITE GOODS"){
      //   console.log("Cy: "+MsgCy);
      //   console.log("El: "+MsgEl);
      //   console.log("MD: "+MsgMD);
...
$(document).ready(function () {
  ...
  $('.hodqasendnotif').on('click', function() {
    var section = $(this).attr('pscsec');
    $.ajax({
      type: "POST",
      url: `${restlocpsc}sendnotifchecker`,
      headers: {
        user: vusrnm,
        token: vtoken,
      },
...
function FungsiSaveValue(){
  ...
  $('.selectpicdqa').on('change', function() {
...
function FungsiUnSignPIC(){

  $('.BtnUnSignPIC').on('click', function() {
    var idcheck = $(this).attr("idchk");
    var sect    = $(this).attr("sect");
    var picc    = $(this).attr("picc");
    Swal.fire({
      title: "<h3>UnSign "+picc+"'s Sign-mark?</h3>",
      confirmButtonText: "Ya",
      showCancelButton: true,
    }).then((result) => {
      if (result.value) {
        $.ajax({
          url: `${restlocpsc}unsignchecker`,
          type: "POST",
          headers: {
            user: vusrnm,
            token: vtoken,
...
function FungsiReturnDesigner(){

  $('.BtnReturnToDesigner').on('click', function() {
    var sect    = $(this).attr("sect");
    Swal.fire({
      title: "<h3>Return to Designer for section: "+sect+"?</h3>",
      text: 'Fasilitas ini berfungsi untuk mengembalikan kondisi Product-Spec yang bersangkutan ke Designer meskipun posisi masih dalam pengecekan DQA yang masih NOT OK maupun sudah OK selama Product-Spec tsb belum di RELEASE.',
      confirmButtonText: "Ya",
      showCancelButton: true,
    }).then((result) => {
      if (result.value) {
        $.ajax({
          url: `${restlocpsc}returndesigner`,
          type: "POST",
          headers: {
            user: vusrnm,
            token: vtoken,
          ...
          success: function (data) {
            var obj = JSON.parse(data);
            Swal.fire({
              text: "Berhasil Return to Designer !",
              timer: 500,
              toast: true,
              showConfirmButton: false,
              showClass: {
...
function FungsiReturnPL(){

  $('.BtnReturnToPL').on('click', function() {
    Swal.fire({
      title: "<h3>Return to Project Leader?</h3>",
      text: 'Fasilitas ini berfungsi untuk mengembalikan kondisi Product-Spec yang bersangkutan ke Project-Leader karena dimungkinkan ada perubahan PIC Designer oleh PL. Ketika berhasil dikembalikan maka step PSC ini mundur ke PL dan diperlukan Sign ulang oleh Designer dan Head of yang bersangkutan. Tombol akan selalu aktif selama Product-Spec tsb belum di RELEASE.',
      confirmButtonText: "Ya",
      showCancelButton: true,
    }).then((result) => {
      if (result.value) {
        $.ajax({
          url: `${restlocpsc}returnpl`,
          type: "POST",
          headers: {
            user: vusrnm,
            token: vtoken,
