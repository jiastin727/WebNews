...
function HOsign(){
  $('.hosign').click(function(){
  let encryption = new Encryption();
  var rl = $(this).attr('rl');
    var nm = $(this).attr('nm');
    var dlink = encryption.encrypt(idpsc, NonceValue);
    Swal.fire({
      title: "<h3><i>Sign-Check</i> Product Spec ini ?</h3>",
      // icon: "info",
  ...
  $.ajax({
    ...
    success: function (response){
      ...
      if (isEmpty(obj)) {
        ...
        if((
           (Sts === false && Msg === 'NULL' && sign_pl === "NOT SIGN") ||
           (Sts === false && Msg === 'NA'   && sign_pl === "NOT SIGN" && sign_hod === "NOT SIGN") ||
           (Sts === false && Msg === 'NOT OK') ) && (
           (Udecrypt === pic_c) ||
           (Udecrypt === pic_e) ||
           (Udecrypt === pic_m) ||
           (Udecrypt === pic_i) ||
            ...
            if (typeof obj["read_content"][a]['nodes'] !== "undefined") {
              ...
              for (b = 0; b < jmlDatab; b++) {
                ...
                if(v_res2 === "OK"){
                  ...
                  var iconresult2 = "<div class='ui tiny left pointing red circular label'>NOT OK</div>";
                } else if ((v_res2 === "MODIF") && (v_cek2 !== null)){
                  var iconresult2 = "<div class='ui tiny left pointing orange circular label'>MODIF</div>";
                } else {
                  var iconresult2 = "";
                }
                ...
                if (typeof obj["read_content"][a]['nodes'][b]['nodes'] !== 'undefined'){
                  ...
                  for (c = 0; c < jmlDatac; c++) {
                    ...
                    if(v_res3 === "OK"){
                      ...
                      var iconresult3 = "<div class='ui tiny left pointing red circular label'>NOT OK</div>";
                    } else if ((v_cek3 === "MODIF") && (v_cek3 !== null)){
                      var iconresult3 = "<div class='ui tiny left pointing orange circular label'>MODIF</div>";
                    } else {
                      var iconresult3 = "";
                    }
                    ...
                    if (typeof obj["read_content"][a]['nodes'][b]['nodes'][c]['nodes'] !== 'undefined'){
                      ...
                      for (d = 0; d < jmlDatad; d++) {
                        ...
                        if(v_res4 === "OK"){
                          ...
                          var iconresult4 = "<div class='ui tiny left pointing red circular label'>NOT OK</div>";
                        } else if ((v_cek4 === "MODIF") && (v_cek4 !== null)){
                          var iconresult4 = "<div class='ui tiny left pointing orange circular label'>MODIF</div>";
                        } else {
                          var iconresult4 = "";
                        }
                        ...
                        if (typeof obj["read_content"][a]['nodes'][b]['nodes'][c]['nodes'][d]['nodes'] !== 'undefined'){
                          ...
                          for (e = 0; e < jmlDatae; e++) {
                            ...
                            if(v_res5 === "OK"){
                              ...
                              var iconresult5 = "<div class='ui tiny left pointing red circular label'>NOT OK</div>";
                            } else if ((v_cek5 === "MODIF") && (v_cek5 !== null)){
                              var iconresult5 = "<div class='ui tiny left pointing orange circular label'>MODIF</div>";
                            } else {
                              var iconresult5 = "";
                            }
...
$('#BtnSignHOG').click(function(){
  let encryption = new Encryption();
  var nm = $(this).attr('nm');
  var dlink = encryption.encrypt(idpsc, NonceValue);
  Swal.fire({
    title: "<h3>Approve Product Spec ini ?</h3>",
    confirmButtonText: "Ya",
    showCancelButton: true,
  }).then((result) => {
    if (result.value) {
      $.ajax({
        type: "POST",
        url: `${restlocpsc}sign_groupproduct`,
        headers: {
          user: vusrnm,
          token: vtoken,
        },
...
$('#SignEngPsc').on('click', function() {
  let encryption = new Encryption();
  var ho     = $('#PSCHOName').val();
  var role   = $('#pscsignrole').val();
  var pic   = $('#pscsignname').val();
  var dlink = encryption.encrypt(idpsc, NonceValue);
  $.ajax({
    url: `${restlocpsc}sign_engineer`,
    type: "POST",
    headers: {
      user: vusrnm,
      token: vtoken,
...
$('#RejectPSCHOG').on('click', function() {
  var sect     = $('#PSCRejectSection').val();
  var reason   = $('#RejectReason').val();
  $.ajax({
    url: `${restlocpsc}reject_groupproduct`,
    type: "POST",
    headers: {
      user: vusrnm,
      token: vtoken,
...
// resvis product spec
  $("#BtnRevisi").on("click", function () {
    ...
    Swal.fire({
      title: 'Revisi Product Spec '+nm_psc+' ?',
      // text: 'Revision Note',
      input: 'text',
      inputAttributes: {
        autocapitalize: 'off'
      },
      inputPlaceholder: "Jika Ya, Silakan cantumkan alasan revisi disini..",
      showCancelButton: true,
      confirmButtonText: 'Ya',
      ...
      preConfirm: (login) => {
        ...
        if (login === "") {
          Swal.fire({text: `Mohon isikan alasan merevisi produk spec ini`})
          return false
        }
