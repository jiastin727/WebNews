...
var DPIC     = Ucrypt.decrypt(EPic, NonceValue);
...
function TableListFunc() {
  ...
  var DTables = $("#EANList").DataTable({
    ...
    columns: [
      ...
      { data: null,
        render: function (data) {
          if(Jdecrypt =="HEAD OF" || DPIC == "PICREG"){
            ...
            if(Jdecrypt =="HEAD OF"){
              ...
              if ((data['Status'] == "ACTIVE") && (data['PIC_EAN'] == ""))  {
                return `<a IdNumE="`+data['id']+`" class="ui small green label EditEAN">Edit</a>`;
              } else if ((data['Status'] == "ACTIVE") && (data['PIC_EAN'] == Udecrypt))  {
      ...
      { data: null,
        render: function (data) {
          if ((data['status'] == "OUTSTANDING"))  {
              return `<a IdNum="`+data['id']+`" class="ui small blue label SetDoneOS">Done</a><a IdNum="`+data['id']+`" class="ui small red label DeleteOS">Delete</a>`;
          } else {
              return ``;
          } 
        },
...
$("#OS_EANList").on("click", ".DeleteOS", function () {
  var IdNum = $(this).attr("IdNum");
  if (IdNum){
    Swal.fire({
      title: "<h3>Hapus outsatnding ini ?</h3>",
      confirmButtonText: "Ya",
      showCancelButton: true,
    }).then((result) => {
      if (result.value) {
...
$("#EditEANCodePIC").on("change", function () {
  var IdEAN = $('.EditId').val();
  $.ajax({
    type    : "POST",
    url     : `${restlocean}SaveEANCodeChange`,
    headers : {
      user  : vusrnm,
      token : vtoken
    },
