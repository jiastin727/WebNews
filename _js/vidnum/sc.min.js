...
function TableListFunc(FSearch, FYear, FNama, FBagian) {
  $("#SCList").DataTable({
    ...
    buttons: [
      {
        ...
        action: function (e, dt, node, config) {
          $("#SCList").DataTable().page.len(10).draw();
        },
      },
      {
        text: "25",
        action: function (e, dt, node, config) {
          $("#SCList").DataTable().page.len(25).draw();
        },
      },
      {
        text: "50",
        action: function (e, dt, node, config) {
          $("#SCList").DataTable().page.len(50).draw();
        },
      },
      {
        text: "100",
        action: function (e, dt, node, config) {
          $("#SCList").DataTable().page.len(100).draw();
        },
      },
      {
        text: "1000",
        action: function (e, dt, node, config) {
          $("#SCList").DataTable().page.len(1000).draw();
        },
      },
    ],
    destroy: true,
    ...
    columns: [
      ...
      { data: 5 },
      {
        ...
        render: function (data) {
          return "<div class='text-wrap width-100'>"+data[7]+"</div>";
        },
      },
      { data: 6 },
      { data: 10 },
      { data: null,
        render: function (data, type, row) {
          if ( data[10] == Udecrypt ) {
            return `<i class="edit outline editsc iconcoloredit icon" id="${data[0]}" num="${data[4]}" typ="${data[7]}" des="${data[6]}" rev="${data[5]}">`;
          } else {
            return "";
          }
        },
    ...
    columnDefs: [
      { className: "center aligned", targets: [0,1,2,4,8] },
      { className: "collapsing", targets: [0,1,3] },
      { orderable: false, targets: [8] },
    ],

    ajax: {
      url: `${restlocvideonum}table_sc`,
      type: "POST",
      headers: {
        user: vusrnm,
        token: vtoken,
...
$(document).ready(function () {
  ...
  $("#SCList").on("click", ".editsc", function () {
    var id  = $(this).attr("id");
  ...
  $("#SCList").on("click", ".viewdoc", function () {
    var num = $(this).attr("num").replaceAll("/", ".");
    var rev = $(this).attr("rev");
    var ext = $(this).attr("ext");
    window.open("../RND_Upload/VideoDoc/SC_"+num+rev+"."+ext, "_blank");
  });
...
$('#savedata').on('click', function(e){
  ...
  var idesc = $('#inputdesc').val();
  $.ajax({
    type: "POST",
    url: `${restlocvideonum}create_sc`,
    headers : {
      user  : vusrnm,
      token : vtoken
    },
    ...
    success: function (response){
      ...
      if(isEmpty(obj)) {
        ...
        var json_str = getCookie('FilterSC');
...
$('#saveedit').on('click', function(e){
  ...
  var idesc = $('#einputdesc').val();
  $.ajax({
    type: "POST",
    url: `${restlocvideonum}update_sc`,
    headers : {
      user  : vusrnm,
      token : vtoken
    },
        ...
        $.ajax({
          ...
          success: function (response){
            ...
            if(isEmpty(obj)) {
              ...
              Swal.fire('Berhasil','Nomor berhasil direvisi.','success');
              var d = new Date();
              var yearnow = d.getFullYear();
              var json_str = getCookie('FilterSC');
              var arr = JSON.parse(json_str);
...
$("#saveupload").on("click", function () {
  ...
  var idsc    = $('#uinputid').val();
  var numsc   = $('#uinputnum').val();
  var form_data = new FormData();
  form_data.append("file", file_data);
  form_data.append("idsc", idsc);
  form_data.append("numsc", numsc);
  $.ajax({
    type: "POST",
    dataType: "text",
    contentType: false,
    processData: false,
    url: `${restlocvideonum}upload_sc`,
    headers: {
      user: vusrnm,
      token: vtoken,
    },
...
if (sfilter) {
  $("#fcari_sc").val(sfilter);
  var sfilterstart = sfilter;
} else {
  var sfilterstart = "";
}
if (tfilter) {
  $("#fthn_sc").dropdown("set selected", tfilter);
  var tfilterstart = tfilter;
} else {
  var tfilterstart = yearnow;
}
if (mfilter) {
  $("#fmn_sc").dropdown("set selected", mfilter);
...
setCookie("FilterSC", JsonFilter, 5);

//Load Table
TableListFunc( sfilterstart, tfilterstart, mfilterstart);

//RESPON KETIKA INPUT PENCARIAN DI KETIK
$("#fcari_sc").on("keyup", function () {
  var FilterCari = $("#fcari_sc").val();
  var FilterTahun = $("#fthn_sc").val();
  var FilterNama = $("#fmn_sc").val();
...
$(".resetfilter").on("click", function () {
  ...
  var yearnow = d.getFullYear();
  $("#fcari_sc").val("");
  $("#fthn_sc").dropdown("set selected", yearnow);
  $("#fmn_sc").dropdown("set selected", "ALL");
